...
const hentaiFox = function(commands, keyword) {
	const url = 'https://hentaifox.com/search/?q=' + keyword;
...
module.exports = class SearchCommand extends commando.Command {
	...
	async run(commands, info) {
		...
		switch (curInfo[0]) {
			case 'nhentai':
				return commands.reply(`Command under construction. Harap bersabar`);
			case 'hentaifox':
				hentaiFox(commands, curInfo[1]);
				break;
			case 'mangafox':
				return commands.reply(`Command under construction. Harap bersabar`);
			default:
				return commands.reply(`Invalid command! Harus salah satu dari NHENTAI, HENTAIFOX, atau MANGAFOX`);
		}
	}
};
